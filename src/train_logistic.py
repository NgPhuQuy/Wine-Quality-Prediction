...
# Thư viện xử lý imbalanced data
...
# ==========================================
# FUNCTION METRIC (Tích hợp Threshold)
# ==========================================
...
# ==========================================
configs = [
    # --- NHÓM GIỮ NGUYÊN 2 HẠT GIỐNG TỐT NHẤT CỦA BẠN ---
    {"name": "logistic_run3", "metric": "recall", "class_weight": None, "smote": False, "threshold": 0.2, "penalty": "l2", "desc": "GIỮ LẠI RUN 3: Hạ mạnh ngưỡng xuống 0.2 để bùng nổ Recall."},
    {"name": "logistic_run5", "metric": "recall", "class_weight": {0: 1, 1: 3}, "smote": False, "threshold": 0.5, "penalty": "l2", "desc": "GIỮ LẠI RUN 5: Phạt cân bằng vừa đủ 1:3 với ngưỡng chuẩn 0.5."},
    
    # --- NHÓM PHÁT TRIỂN QUANH RUN 3 (Dò tìm Threshold mượt hơn) ---
    # (Giữ class_weight=None, rà soát vùng đệm giữa 0.2 và 0.4)
    {"name": "logistic_run1", "metric": "recall", "class_weight": None, "smote": False, "threshold": 0.3, "penalty": "l2", "desc": "Vùng đệm 0.3: Tìm điểm cân bằng tốt hơn mức 0.2 của Run 3."},
    {"name": "logistic_run2", "metric": "recall", "class_weight": None, "smote": False, "threshold": 0.25, "penalty": "l2", "desc": "Nhích nhẹ lên 0.25: Xem F1-score có cải thiện so với mức 0.2 không."},
    
    # --- NHÓM PHÁT TRIỂN QUANH RUN 5 (Dò tìm Class Weight mượt hơn) ---
    # (Giữ Threshold=0.5, rà soát nấc thang phạt tinh vi hơn)
    {"name": "logistic_run4", "metric": "recall", "class_weight": {0: 1, 1: 2.5}, "smote": False, "threshold": 0.5, "penalty": "l2", "desc": "Nấc đệm 1:2.5: Giảm nhẹ độ phạt so với Run 5 để kéo lại Precision."},
    {"name": "logistic_run6", "metric": "recall", "class_weight": {0: 1, 1: 3.5}, "smote": False, "threshold": 0.5, "penalty": "l2", "desc": "Nấc đệm 1:3.5: Tăng nhẹ độ phạt so với Run 5 để đẩy thêm Recall."},
    
    # --- NHÓM COMBO TỐI ƯU (SIÊU PHẨM PHỐI HỢP) ---
    {"name": "logistic_run7", "metric": "recall", "class_weight": {0: 1, 1: 2.5}, "smote": False, "threshold": 0.3, "penalty": "l2", "desc": "Sự kết hợp hoàn hảo: Phạt nhẹ 1:2.5 và hạ ngưỡng vừa vặn xuống 0.3."}
]
...
for config in configs:
    ...
    # FIX TẬN GỐC WARNINGS TRÊN SKLEARN MỚI
    ...
    # NÂNG CẤP 1: Tính toán đầy đủ và làm tròn 4 chữ số
    ...
    # NÂNG CẤP 2: Đẩy đầy đủ 9 thông số vào mảng lịch sử
    ...
    # ------------------------------------------
    # Log WandB (Kèm theo overfit_gap để vẽ biểu đồ)
    # ------------------------------------------
    wandb.log({
        ...
        "test_auc": test_auc,
        "overfit_gap": overfit_gap, # Vẽ Gap lên WandB
        "test_f1": t_f1,
    ...
    # Save model
    joblib.dump(best_model, f"models/{config['name']}.joblib")
...
print("Ý nghĩa: Có khoảng cách giữa CV và Test (Overfit Gap) nhỏ nhất, chứng tỏ không bị học vẹt!")
print("="*80)